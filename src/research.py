...
pick = Weapon('Pick', 20, 4, 2.2, 4, 1, 1, 1, reload_time=8) 
hatchet = Weapon('Hatchet', 15, 4, 1.8, 2.5, 1, 1.6, 1, reload_time=2) 
#Aztec - means obsidan blade
macuahuitl = Weapon('Macuahuitl', 60, 7, 3.0, 6, 1.25, 1.5, 1, reload_time=8) 
falchion = Weapon('Falchion', 40, 5, 1.5, 8, 1, 1.25, 1, reload_time=6) 


sabre = Weapon('Sabre', 50, 7, 1.8, 7, 0.5, 2.35, 1, reload_time=3) 
cleaver = Weapon('Cleaver', 35, 5, 3, 8, 0, 0.5, 0, reload_time=6) 
sickle = Weapon('Sickle', 10, 3, 1.4, 1, 2, 1, 1.25, reload_time=1) 
katana = Weapon('Katana', 125, 7, 2.4, 7, 0.25, 2, 0.25, reload_time=4) 
war_axe = Weapon('War Axe', 50, 8, 1.8, 10, 0, 1.2, 0.5, reload_time=12) 
battle_axe = Weapon('Battle Axe', 65, 9, 1.4, 12, 0, 1.0, 0.35, reload_time=14) 
knife = Weapon('Knife', 5, 5, 0.45, 1.5, 0.5, 1.5, 0.5, reload_time=1) 
hand_axe = Weapon('Hand Axe', 10, 7, 0.6, 2.2, 0.9, 1.9, 0.9, reload_time=8) 
...
war_hammer = Weapon('War hammer', 250, 14, 1.6, 15, 0, 2, 0, reload_time=13)
short_spear = Weapon('Short Spear', 50, 12, 1.2, 5, 2, 1.8, 1.2, reload_time=9)
scimitar = Weapon('Scimitar', 80, 10, 2.0, 5, 0, 2, 0.5, reload_time=4)
estoc = Weapon('Estoc', 175, 12, 2.2, 8, 2, 1, 1.5, reload_time=8)
gladius = Weapon('Gladius', 120, 8, 1.8, 7, 1, 2, 1.5, reload_time=7)
daikatana = Weapon('Dai Katana', 220, 12, 1.8, 7, 0, 2.5, 0.5, reload_time=8)
nodachi = Weapon('No Dachi', 250, 14, 1.9, 8, 0, 3, 0.5, reload_time=10)
...
zweihander = Weapon('Zweihander', 350, 14, 2.0, 10, 0.2, 2, 0.5, reload_time=9)
flamberge = Weapon('Flamberge', 320, 14, 2.0, 9, 0.5, 2.5, 0.2, reload_time=8)
clamshell = Weapon('Clamshell', 300, 14, 2.0, 8, 3,   1.7, 1.2, reload_time=8)
longsword = Weapon('Longsword', 160, 14, 2.0, 8, 2.5, 1, 1, reload_time=7)
#German - stick with a spiked tip
goedendag = Weapon('Goedendag', 20, 11, 1.1, 4, 2, 1.5, 1, reload_time=7)
...
bardiche = Weapon('Bardiche', 120, 15, 0.8, 10, 1, 1, 0.6, reload_time=11)
voulge = Weapon('Voulge', 105, 15, 0.9, 8, 1, 1.5, 0.8, reload_time=9)
naginata = Weapon('Naginata', 220, 14, 2, 4, 0.5, 3, 1.5, reload_time=8)
...
lance = Weapon('Lance', 180, 23, 1.5, 9, 1, 2, 1, reload_time=11)
...
sidearm_list = [pick, hatchet, falchion, sabre, cleaver, sickle, knife, hand_axe, tanto, dagger, club, mace, kopis, hammer, rondel, dirk, staff, shortsword, axe, spear]
...
throwing_knives = Weapon('Throwing Knives', 15, 220, 0, 1.5, 0, 1.8, 1, reload_time=50, ammunition=5, projectile_speed=6)
throwing_axes = Weapon('Throwing Axes', 15, 250, 0, 5, 0, 1.8, 1, reload_time=70, ammunition=3, projectile_speed=5)
throwing_daggers = Weapon('Throwing Daggers', 15, 220, 0, 2, 0, 1.8, 1, reload_time=50, ammunition=5, projectile_speed=6)
jarid = Weapon('Jarids', 15, 250, 0, 4, 0, 1.8, 1, reload_time=60, ammunition=10, projectile_speed=6)
stones = Weapon('Stones', 15, 200, 0, 0.2, 0, 1.8, 1, reload_time=30, ammunition=100, projectile_speed=3)
darts = Weapon('Darts', 15, 200, 0, 1, 0, 1.8, 1, reload_time=20, ammunition=30, projectile_speed=5)
pilum = Weapon('Pilum', 15, 400, 0, 7, 0, 2, 1, reload_time=10, ammunition=10, projectile_speed=6)

recurve_bow = Weapon('Recurve Bow', 200, 320, 0.8, 7, 1, 3, 1, reload_time=90, ammunition=15, projectile_speed=14)
war_bow = Weapon('War Bow', 185, 400, 0.5, 8, 0.5, 2, 0.5, reload_time=100, ammunition=15, projectile_speed=13)
...
arbalest = Weapon('Arbalest', 300, 500, 1.5, 12, 1, 1, 1, reload_time=250, ammunition=15, projectile_speed=30)
hunting_crossbow = Weapon('Hunting Crossbow', 200, 400, 1.5, 8, 1, 1, 1, reload_time=150, ammunition=15, projectile_speed=15)

handgun = Weapon('Handgun', 500, 350, 1.5, 30, 1, 1, 1, reload_time=400, ammunition=15, projectile_speed=40)
flintlock_pistol = Weapon('Flintlock Pistol', 400, 200, 1.5, 20, 1, 1, 1, reload_time=350, ammunition=15, projectile_speed=30)
flintlock_rifle = Weapon('Flintlock Rifle', 600, 380, 1.5, 33, 1, 1, 1, reload_time=450, ammunition=15, projectile_speed=50)
matchlock_rifle = Weapon('Matchlock Rifle', 650, 400, 1.5, 35, 1, 1, 1, reload_time=470, ammunition=15, projectile_speed=50)
blunderbuss = Weapon('Blunderbuss', 550, 200, 1.5, 50, 1, 1, 1, reload_time=500, ammunition=15, projectile_speed=30)
arquebus = Weapon('Arquebus', 700, 400, 1.5, 40, 1, 1, 1, reload_time=500, ammunition=15, projectile_speed=45)
musket = Weapon('Musket', 1000, 500, 1.5, 50, 1, 1, 1, reload_time=600, ammunition=15, projectile_speed=60)


#Ranged Siege - Kenny Additions
trebuchet = Weapon('Trebuchet', 950, 600, 1.5, 60, 1, 1, 1, reload_time=800, ammunition=5, projectile_speed=30)
scorpio = Weapon('Scorpio', 700, 500, 1.5, 40, 1, 1, 1, reload_time=700, ammunition=20, projectile_speed=60)
ballista = Weapon('Ballista', 600, 500, 1.5, 42, 1, 1, 1, reload_time=800, ammunition=20, projectile_speed=50)
onager = Weapon('Onager', 750, 550, 1.5, 50, 1, 1, 1, reload_time=700, ammunition=10, projectile_speed=25)
mortar = Weapon('Mortar', 850, 600, 1.5, 55, 1, 1, 1, reload_time=750, ammunition=10, projectile_speed=20)
cannon = Weapon('Cannon', 1000, 650, 1.5, 60, 1, 1, 1, reload_time=1000, ammunition=10, projectile_speed=45)
catapult = Weapon('Catapult', 750, 530, 1.5, 60, 1, 1, 1, reload_time=600, ammunition=30, projectile_speed=25)
...
ranged_weapon_list = [scorpio, ballista, onager, mortar, cannon, catapult, musket, arquebus, blunderbuss, matchlock_rifle, flintlock_rifle, flintlock_pistol, arbalest, hunting_crossbow, recurve_bow, war_bow, self_bow, hunting_bow, throwing_knives, throwing_axes, throwing_daggers, jarid, stones, darts, sling, atlatl, javelin, shortbow, longbow, bow, crossbow, sling_staff, handgun, trebuchet]
...
byrnie = Armor('Byrnie', 600, 1.7, 8, 0.75)

desert_mail = Armor('Desert Mail', 1000, 1, 6, 1)

surcoat = Armor('Surcoat', 1300, 2, 9, 0.20)

brigandine = Armor('Brigandine', 1500, 2, 11, 0.25)
corrazina = Armor('Corrazina', 1600, 2, 11, 0.50)
scale_armor = Armor('Scale Armor', 1800, 1.5, 12, 0)

coat_plate = Armor('Coat of Plates', 1900, 2, 11, 0.25)
cuir_bouilli = Armor('Cuir Bouilli', 1700, 2, 9, 0.45)

royal_armor = Armor('Royal Armor', 2000, 2, 15, 0.1)
royal_mail = Armor('Royal Mail', 1400, 1.7, 9, 1)

holy_armor = Armor('Holy Armor', 800, 1, 6, 0.35)
holy_mail = Armor('Holy Mail', 900, 1, 6, 0.85)


armor_list = [lorica, byrnie, desert_mail, surcoat, brigandine, corrazina, scale_armor, coat_plate, cuir_bouilli, royal_armor, royal_mail, holy_armor, holy_mail, tunic, robe, fur_coat, jerkin, linen_shirt, aketon, tabard, ragged_armor, tribal_wrappings, gambeson, hauberk, lamellar, cloth_armor, padded_armor, leather_armor, wood_armor, chainmail, plate]

armor_list = [lorica, byrnie, desert_mail, surcoat, brigandine, corrazina, scale_armor, coat_plate, cuir_bouilli, wood_armor, hauberk, lamellar, chainmail, plate]
