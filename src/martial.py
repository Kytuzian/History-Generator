...
class Soldier:
    def __init__(self, unit, name, health, strength, ranged, discipline, canvas):
        self.unit = unit
    ...
    def get_attack(self):
        ...
        if self.unit.soldier_type.originally_ranged:
            normal_attack = random.randint(0, self.strength // 2)
        else:
            normal_attack = random.randint(0, self.strength)

        result = normal_attack - fatigue_loss
...
class Unit:
    def __init__(self, soldier_type, soldiers, canvas):
        ...
        self.ammunition = 100 + len(soldiers) * soldier_type.discipline
...
class Battle():
    def __init__(self, force_a, a_army, force_b, b_army, city, battle_over):
        self.a = force_a
        self.b = force_b
    ...
    def setup_army(self, army, force, color, (xmin, xmax), (ymin, ymax), limit):
       ...
       # print("Setting up a max of {} soldiers of {} for this unit type.".format(limit, army.number))
        ...
        for soldier in xrange(army.number):
            ...
            force[-1].soldiers.append(Soldier(force[-1], army.name, army.health, army.strength, army.ranged, army.discipline, self.canvas))
                ...
                if p.skip_step <= 0:
                    if p.kill_range > 0:
                        ...
                        if utility.collided((x, y, PROJECTILE_RADIUS), (tx, ty, TROOP_RADIUS)):
                            ...
                            damage = random.randint(0, p.strength)
    ...
    def handle_units(self, force, army, proj, enemy_force, enemy_army, color):
        for current_unit in force:
            ...
            #     current_unit.calculate_shoot_position()
                ...
                if soldier.ranged:
                    ...
                    if soldier.shoot > soldier.shoot_counter:
                        if d > 0 and current_unit.ammunition > 0:
                            ...
                            proj.append(Projectile(((tx - x) / d, (ty - y) / d), soldier.strength, soldier.target))
                        ...
                        soldier.shoot += random.randint(0, int(sqrt(soldier.discipline)))
                else:
